...
describe("GET /api/calendario", () => {
  it("incluye un pill 'grano' para un batch con jars sin colonizar", async () => {
    const batch = await makeBatch({ fechaInicio: "2030-01-01T00:00:00.000Z", diasEsperados: 10 });
  ...
  it("oculta el pill 'grano' cuando todos los jars ya fueron colonizados", async () => {
    const batch = await makeBatch({ fechaInicio: "2030-01-01T00:00:00.000Z", diasEsperados: 10 });
    await colonizarJars(batch.jars);

    const { json } = await callRoute(GET, { searchParams: { year: "2030", month: "1" } });
  ...
  it("no aparece al pedir un mes distinto al de la fecha esperada, si al pedir el correcto", async () => {
    await makeBatch({ fechaInicio: "2030-01-01T00:00:00.000Z", diasEsperados: 10 });

    const mesIncorrecto = await callRoute(GET, { searchParams: { year: "2030", month: "2" } });
    expect(
      mesIncorrecto.json.data.lotePills.find((p: { tipo: string }) => p.tipo === "grano")
    ).toBeUndefined();

    const mesCorrecto = await callRoute(GET, { searchParams: { year: "2030", month: "1" } });
    expect(
  ...
  it("incluye un pill 'incubacion' para un recipiente incubando, con href al lote", async () => {
    const { batch, recipiente } = await makeBatchConRecipiente();
    ...
    const { status, json } = await callRoute(GET, { searchParams: { year, month } });
  ...
  it("al fructificar el recipiente, el pill 'incubacion' desaparece y aparece 'fructificacion'", async () => {
    const { recipiente } = await makeBatchConRecipiente();
    ...
    const { json } = await callRoute(GET, { searchParams: { year, month } });
  ...
  it("incluye un pill 'placas' para una clonacion (origen placa) sin colonizar", async () => {
    ...
    const { status, json } = await callRoute(GET, { searchParams: { year: "2030", month: "2" } });
  ...
  it("oculta el pill 'placas' cuando todas las placas ya fueron colonizadas", async () => {
    ...
    await colonizarPlacas(clonacion.placas);

    const { json } = await callRoute(GET, { searchParams: { year: "2030", month: "2" } });
  ...
  it("trae tareas y lotePills sin mezclarse", async () => {
    await makeBatch({ fechaInicio: "2030-01-01T00:00:00.000Z", diasEsperados: 10 });
    await makeTarea({ titulo: "Revisar", fecha: "2030-01-20" });

    const { status, json } = await callRoute(GET, { searchParams: { year: "2030", month: "1" } });
